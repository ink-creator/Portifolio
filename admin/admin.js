...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const loginErrors = {
    acesso_negado:
      "Esta conta GitHub não tem acesso ao admin. Entre com uma conta autorizada pelo responsável pelo portfólio.",
  ...
  if (!API || API.includes("SEU-WORKER")) {
    ...
    status.textContent =
      "Seu editor já está disponível. Para publicar, conclua a configuração e preencha a URL pública em admin/cloudflare.js.";
    return;
  }
  ...
  async function checkSession() {
    if (callbackError) {
      setAuthenticated(
        null,
        loginErrors[callbackError] ||
          "Não foi possível entrar. Tente novamente com uma conta autorizada.",
      );
      return;
    }
    const token = getToken();
    ...
    try {
      ...
      status.textContent =
        "Não foi possível verificar sua sessão. Confira a conexão e tente recarregar a página.";
    }
  }
  ...
  logout.addEventListener("click", () => {
    ...
    setAuthenticated(
      null,
      "Você saiu do painel. A sessão do site GitHub continua aberta; escolha a conta desejada ao entrar novamente.",
    );
  });
