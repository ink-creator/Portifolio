document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (query.get("preview") === "1") {
      ...
      if (!project) {
        ...
        if (legacy) {
          ...
          if (parsed.id === id) project = parsed;
        }
      }
    }
    ...
    root.innerHTML =
      '<div class="empty-state"><h2>Não foi possível abrir este projeto.</h2><p>Tente carregar a página novamente.</p><a class="button secondary" href="../projetos/">Voltar para projetos</a></div>';
    return;
  }
  if (!project) {
    root.innerHTML =
      '<div class="empty-state"><h2>Projeto não encontrado.</h2><p>Explore os outros projetos da dupla.</p><a class="button primary" href="../projetos/">Explorar projetos →</a></div>';
    return;
  }
  const p = project,
    e = Portfolio.escapeHTML,
    u = Portfolio.assetURL,
    media = p.media || {},
    links = p.links || {};
  ...
  const actions = [
    ...
    .map(
      ([key, title]) =>
        '<a class="button secondary" target="_blank" rel="noopener" href="' +
        e(links[key]) +
        '">' +
        title +
        " ↗</a>",
