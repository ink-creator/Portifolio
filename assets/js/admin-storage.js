  ...
  function open() {
    if (!database)
      database = new Promise((resolve, reject) => {
        ...
        request.onerror = () =>
          reject(new Error("Não foi possível abrir os rascunhos neste navegador."));
        request.onblocked = () =>
          reject(new Error("Feche outras abas do editor e tente novamente."));
      });
    return database;
  }
  async function run(mode, action) {
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.onabort = transaction.onerror = () =>
        reject(
          new Error(
            "Não foi possível guardar o rascunho. Verifique o espaço disponível no navegador.",
          ),
        );
    });
  }
