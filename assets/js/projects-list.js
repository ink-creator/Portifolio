document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    filters.innerHTML = ["all", ...types]
      .map(
        (type, i) =>
          ...
          escape(type) +
          '">' +
          escape(type === "all" ? "Todos" : Portfolio.typeNames[type] || type) +
          "</button>",
      )
      .join("");
    const render = (type) =>
      Portfolio.renderProjectGrid(
        list,
    ...
    list.innerHTML =
      '<div class="empty-state"><h2>Não foi possível carregar os projetos.</h2><p>Tente novamente em instantes.</p><button class="button secondary" id="retry-projects">Tentar novamente</button></div>';
