...
async function publish(env, projects) {
  ...
  if (!newCommitResponse.ok)
    throw new Error(`Falha ao criar commit: ${await newCommitResponse.text()}`);
  ...
  if (!updateResponse.ok)
    throw new Error("O repositório mudou durante a publicação. Recarregue e tente novamente.");
...
async function handle(request, env) {
  ...
  if (url.pathname === "/auth/callback" && request.method === "GET") {
    ...
    if (!meResponse.ok || !isAdmin(account.login, env))
      return redirect(`${adminUrl}#error=acesso_negado`);
  ...
  if (url.pathname === "/auth/me" && request.method === "GET") {
    return user
      ? json(request, env, { authenticated: true, user })
  ...
  fetch(request, env) {
    return handle(request, env).catch((error) =>
      json(
        request,
        env,
        { error: "publish_error", detail: String(error.message || error) },
        error.status || 500,
      ),
    );
  },
