...
export function parseMedia(value, kind) {
  ...
  if (!value.startsWith("data:")) {
    if (!/^(?:https?:\/\/|assets\/)/i.test(value))
      throw new Error(
        "Use um arquivo enviado pelo painel, caminho assets/ ou link HTTP(S) para a mídia.",
      );
    return null;
  }
  const match = value.match(/^data:([^;,]+);base64,([A-Za-z0-9+/]+={0,2})$/);
  const extension = match && types[kind][match[1]];
  if (!extension || match[2].length % 4 !== 0)
    throw new Error(`Formato de ${kind === "video" ? "vídeo" : "imagem"} inválido.`);
  ...
  if (bytes > limit)
    throw new Error(
      `${kind === "video" ? "Vídeo" : "Imagem"} excede o limite de ${limit / 1024 / 1024} MB.`,
    );
...
export async function readPublishBody(request) {
  const tooLarge = () =>
    Object.assign(new Error("Publicação maior que 25 MB. Envie menos mídias por vez."), {
      status: 413,
    });
  ...
  try {
    ...
    throw Object.assign(new Error("JSON de publicação inválido."), { status: 400 });
  }
}
