...
async function panel({
  hash = "",
  token = null,
  fetch = async () => Response.json({ authenticated: true, user: { login: "ink-creator" } }),
  preparePublish,
  busy = false,
} = {}) {
  const nodes = new Map();
  ...
  const admin = {
    ...
    setBusy: (value) => {
      busy = value;
      listeners.get("portfolio-editor-state")?.();
    },
    preparePublish:
      preparePublish ||
      (async () => {
...
test("recusa de outra conta permanece visível e não reaproveita sessão anterior", async () => {
  const app = await panel({
    hash: "#error=acesso_negado",
    token: "old-test-session",
    fetch: () => {
      throw Error("Não deveria consultar a sessão recusada");
    },
  });
...
test("login oferece escolha da conta e mantém state e PKCE", async () => {
  const response = await worker.fetch(new Request("https://worker.test/auth/login"), {
    ADMIN_URL: "https://site.test/admin/",
